# Deployment Guide - AWS Assignment WebStore
...
| **Amazon RDS** | PostgreSQL or MySQL database for users, products, and cart data |
...
User Browser
 ...
 [EC2 Instance]
     ...
     +-- Docker Container (Node.js App - Port 8089)
              |
              +---> RDS (users, products, cart_items tables)
              |     Supports both PostgreSQL and MySQL
              |     Set DB_DIALECT=postgres or DB_DIALECT=mysql
              |
              +---> S3 Bucket
...
The application uses **Sequelize ORM** to connect to a database on RDS. It supports both **PostgreSQL** and **MySQL** — set the `DB_DIALECT` environment variable accordingly:

- `DB_DIALECT=postgres` — for RDS PostgreSQL (default)
- `DB_DIALECT=mysql` — for RDS MySQL

The app creates 3 tables automatically on first startup:
...
The connection is configured via environment variables (`DB_HOST`, `DB_PORT`, `DB_NAME`, `DB_USER`, `DB_PASSWORD`).

**Important**: The database named in `DB_NAME` must already exist on the RDS instance. The app creates the tables, not the database itself. You need to connect to RDS and run `CREATE DATABASE cloudmart;` before starting the app.

**Important**: The RDS security group must allow inbound traffic on the database port (5432 for PostgreSQL, 3306 for MySQL) from the EC2 instance's security group.
...
**Requirements**:
- The S3 bucket must allow public read access on `/static/*` and `/products/*` paths so browsers can load images and assets
- The EC2 instance must have IAM permissions to upload (PutObject) and delete (DeleteObject) objects in the S3 bucket
...
On EC2, the EFS file system should be mounted at a directory (e.g., `/logs`). Docker maps this into the container via the volume mount in docker-compose:
...
**Note**: The EFS mount directory must be writable by the container. If logs are not appearing, check the directory permissions on the EC2 host.
...
### Session Handling and Auto Scaling

This application stores user sessions **in memory** (Node.js process). This means:

- If you run **multiple EC2 instances** behind an ALB (Auto Scaling), a user logged in on Instance A will appear logged out if their next request goes to Instance B.
- **Solution**: Enable **sticky sessions** on your ALB target group. This ensures all requests from the same user are routed to the same instance.
  - Target Group → Attributes → Stickiness → Enable → Application-based cookie or Duration-based
- Without sticky sessions, browsing products (public pages) works fine on any instance, but login/cart/admin features will break across instances.

### Database Support

The application supports both **PostgreSQL** and **MySQL**. Configure via the `DB_DIALECT` environment variable:

| DB_DIALECT | RDS Engine | Default Port | Notes |
|------------|-----------|-------------|-------|
| `postgres` | RDS PostgreSQL | 5432 | Default. Uses SSL in production automatically. |
| `mysql` | RDS MySQL | 3306 | Change `DB_PORT` to `3306` in docker-compose. |

### HTTPS / SSL

This app runs on **HTTP** only. It does not handle SSL termination. If you need HTTPS:
- Configure SSL termination at the **ALB** level
- The app itself should remain on HTTP behind the ALB
...
- Verify RDS security group allows inbound from EC2 security group on the database port
...
```bash
# For PostgreSQL - connect from EC2 using psql
psql -h YOUR-RDS-ENDPOINT -U YOUR-DB-USER -d postgres
CREATE DATABASE cloudmart;
\q

# For MySQL - connect from EC2 using mysql client
...
4. If returning 403 Forbidden, check your S3 bucket policy and Block Public Access settings
...
- Verify IAM role is attached to EC2: `curl http://169.254.169.254/latest/meta-data/iam/security-credentials/`
...
- Restart container after fixing permissions: `docker restart hsenid-cloudmart`
...
| `DB_DIALECT` | Yes | Database type (`postgres` or `mysql`) | `postgres` |
| `DB_HOST` | Yes | RDS endpoint | `mydb.abc.us-east-2.rds.amazonaws.com` |
| `DB_PORT` | Yes | Database port | `5432` (PostgreSQL) or `3306` (MySQL) |
